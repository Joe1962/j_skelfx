...
import cu.cenpalab.siguapa.exceptionhandlers.SQLx;
...
import static cu.cenpalab.siguapa.global.GLOBAL.arguments;
import static cu.cenpalab.siguapa.global.GLOBAL.doExit;
import static cu.cenpalab.siguapa.global.GLOBAL.queryKnownOS;
...
//			}
	...
	private void Login() throws IOException {
		...
		boolean retBool = Protection.checkUsers(GLOBAL.MainClass, null, AppInfo.getTitleString(), "Crear primer usuario (superadmin)...", true);
		if(!retBool) {
			// TODO: ERROR message and exit with error status code...
			MsgWarningOKFX(null, "AVISO", null, "No se creó el primer usuario," + NEW_LINE + "El sistema terminará...");
			Platform.exit();
			System.exit(2);			// Change to the correct exit status code...
		}

		GLOBAL.CurrUser = Protection.doLogin(null, "Login: " + AppInfo.getTitleString());
		if (FLAGS.isLOGGEDIN()) {
