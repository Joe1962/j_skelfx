...
import static cu.cenpalab.siguapa.global.GLOBAL.DBCONFIG;
import cu.cenpalab.siguapa.global.interfaces.CallbackForTabs;
import static cu.jsoft.j_loginfx.global.CONSTS.AESSalt;
import static cu.jsoft.j_loginfx.global.CONSTS.SecKeyStr;
import static cu.jsoft.j_loginfx.global.CONSTS.iv;
...
//		DBDriverList.add("Derby");
	...
	public void loadData(TYP_ConfigDBJSON MyDBConfig) {
		...
		cmbDBType.getSelectionModel().select(thisDBConfig.getDBTYPE());
		...
		String DBPASS = Protection.getDecryptedString(DBPASSEncrypted, new StringBuffer(SecKeyStr).reverse().toString(), iv);
...
//			SUB_Popups.MsgWarningOK(dialogsParent, title, content);
	...
	private boolean saveDBConfig() {
		...
		MyDBConfig.setDBDRIVER("jdbc");
		MyDBConfig.setDBTYPE(cmbDBType.getSelectionModel().getSelectedItem() == null ? "PostgreSQL" : cmbDBType.getSelectionModel().getSelectedItem().trim());
		...
		String DBPASSEncrypted = Protection.getEncryptedString(DBPASS, new StringBuffer(AESSalt).reverse().toString(), new StringBuffer(SecKeyStr).reverse().toString(), iv);
